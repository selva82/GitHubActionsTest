...
describe('processOpenApi', () => {
  it('adds the correct API Gateway extension for every supported integration', () => {
    ...
    const orders = processed.paths['/orders']! as Record<string, Record<string, unknown>>;
    const getIntegration = (operation: Record<string, unknown>) => operation['x-amazon-apigateway-integration'];
    ...
    expect(getIntegration(processed.paths['/orders/lambda']!.post as Record<string, unknown>)).toMatchObject({
      type: 'aws_proxy',
      httpMethod: 'POST',
    ...
    expect(getIntegration(processed.paths['/orders/aws']!.get as Record<string, unknown>)).toMatchObject({
      type: 'aws',
      httpMethod: 'POST',
      credentials: 'arn:aws:iam::123456789012:role/api-gateway'
    });
  ...
  it('does not mutate the raw document or leak internal metadata', () => {
    const processed = processOpenApi(rawDocument);
    expect((rawDocument.paths['/orders'].get as Record<string, unknown>)['x-amazon-apigateway-integration']).toBeUndefined();
  ...
  it('fails with an actionable error for incomplete integration metadata', () => {
    ...
    } } } })).toThrowError(new OpenApiProcessingError(
      "paths['/bad'].get.x-integration.connectionId is required for VPC_LINK"
    ));
  });
});